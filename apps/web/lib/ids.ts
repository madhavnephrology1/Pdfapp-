...
export const paragraphId = (regionIdValue: string, index: number): string =>
  `${regionIdValue}:para${index}`;

export const sentenceId = (paragraphIdValue: string, index: number): string =>
  `${paragraphIdValue}:s${index}`;
